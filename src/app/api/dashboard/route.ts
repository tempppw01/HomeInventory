...
export async function GET() {
  try {
    ...
    const [items, locations, shopping, priceRecords, consumptionLogs, preferences, tasks] = await Promise.all([
      ...
      prisma.householdTask.findMany({ where: { status: "OPEN" }, orderBy: { dueAt: "asc" }, take: 8 }),
    ]);
