...
export async function POST(request: NextRequest) {
  try {
    ...
    // Remote URLs are already provider-readable; only compress browser data URLs.
    // This keeps pasted OSS/CDN image links working while still protecting the
    // provider from oversized local uploads.
    const preparedImages = await Promise.all(images.map(async (image) => ({
      ...image,
      dataUrl: image.dataUrl.startsWith("data:image/") ? await compressAiImageDataUrl(image.dataUrl) : image.dataUrl,
    })));
    const instruction = mode === "receipt" ? "识别这张购物小票，按小票上的每一行商品分别生成物品草稿。只返回 JSON 对象 {\"items\":[...]}，不要 Markdown。每项字段：sourceIndex(固定为0), name, category(日用/食品/饮品/清洁/家电/数码/衣物/户外/其他；药品必须按治疗方向使用医药·感冒发热、医药·消化系统、医药·呼吸过敏、医药·疼痛炎症、医药·皮肤外用、医药·眼耳鼻喉、医药·心脑血管、医药·妇儿健康、医药·慢病管理、医药·医疗用品或医药·其他), type(DURABLE或CONSUMABLE), quantity(数字), unit, price(该行商品总价或单价，数字或null), purchaseDate(YYYY-MM-DD或null), expiryDate(YYYY-MM-DD或null), notes, confidence(0到1)。优先识别商品名、数量、价格和小票日期；无法确定的字段返回 null 或合理默认值。食品、清洁用品等通常为 CONSUMABLE，耐用品 expiryDate 必须为 null。" : "识别图片中清晰可见的家庭物品，每张图片对应一个物品；如果一张图有多个物品，分别列出。只返回 JSON 对象 {\"items\":[...]}，不要 Markdown。每项字段：sourceIndex(图片序号，从0开始), name, category(日用/食品/饮品/清洁/家电/数码/衣物/户外/其他；药品必须按治疗方向使用医药·感冒发热、医药·消化系统、医药·呼吸过敏、医药·疼痛炎症、医药·皮肤外用、医药·眼耳鼻喉、医药·心脑血管、医药·妇儿健康、医药·慢病管理、医药·医疗用品或医药·其他), type(DURABLE或CONSUMABLE), quantity(数字), unit, purchaseDate(YYYY-MM-DD或null), expiryDate(YYYY-MM-DD或null), notes, confidence(0到1)。sourceIndex 必须对应识别该物品的图片；耐用品 expiryDate 必须为 null，不确定的字段保守填写。图片无法判断购买日期时 purchaseDate 返回 null。";
    const content = [{ type: "text", text: instruction }, ...preparedImages.map((image) => ({ type: "image_url", image_url: { url: image.dataUrl, detail: "low" } }))];
    const payload = config.protocol === "anthropic"
      ? { model: config.model, max_tokens: 1800, messages: [{ role: "user", content: [{ type: "text", text: instruction }, ...preparedImages.map((image) => ({ type: "image", source: { type: "base64", media_type: image.dataUrl.match(/^data:(image\/[^;]+);/)?.[1] || "image/jpeg", data: image.dataUrl.split(",")[1] || "" } }))] }] }
