...
export async function POST(request: NextRequest) {
  try {
    ...
    const attachmentContext = messages.flatMap((message: Message) => (message.attachments || []).filter((attachment: Attachment) => attachment.kind !== "image" && attachment.text).map((attachment: Attachment) => `附件 ${attachment.name || "未命名"}：${attachment.text}`)).join("\n");
    ...
    const response = await call(config.protocol === "anthropic" ? { model: config.model, max_tokens: 1000, system, messages: messages.map((message: Message) => ({ role: message.role, content: toAnthropicContent(message) })) } : { model: config.model, messages: [{ role: "system", content: system }, ...messages.map((message: Message) => ({ role: message.role, content: toOpenAiContent(message) }))], temperature: 0.2 });
