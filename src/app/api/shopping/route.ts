  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        await tx.activityLog.create({ data: { action: "SHOPPING_MERGE", itemName: item.name, detail: "合并重复采购项" } });
        return { item, merged: true };
      }
      const item = await tx.shoppingItem.create({ data });
      await tx.activityLog.create({ data: { action: "SHOPPING_CREATE", itemName: item.name, detail: "添加采购项" } });
