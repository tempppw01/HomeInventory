...
export function PrintStudio({ items, onClose }: { items: Item[]; onClose: () => void }) {
  ...
  const printCss = `
    @media print {
      ...
      .print-studio-root { position: static !important; inset: auto !important; display: block !important; min-height: 0 !important; height: auto !important; background: transparent !important; }
      .print-controls { display: none !important; }
      .print-preview-area { position: static !important; display: block !important; width: auto !important; min-height: 0 !important; height: auto !important; overflow: visible !important; padding: 0 !important; background: transparent !important; }
  ...
  return <div className="print-studio-root fixed inset-0 z-[80] flex flex-col bg-black/45 backdrop-blur-sm lg:flex-row">
    ...
    <div className="print-preview-area flex-1 overflow-auto p-4 lg:p-8" style={{ background: "#d9dae0" }}>
...
function NumberInput({ value, min, max, step = "1", integer = false, onChange }: { value: number; min: number; max: number; step?: string; integer?: boolean; onChange: (value: number) => void }) {
  ...
  const commit = (raw: string) => { const parsed = Number(raw); if (!Number.isFinite(parsed)) { setDraft(String(value)); return; } const next = Math.min(max, Math.max(min, integer ? Math.floor(parsed) : parsed)); setDraft(String(next)); onChange(next); };
  return <input className="input" type="text" inputMode={integer ? "numeric" : "decimal"} autoComplete="off" value={draft} onChange={(event) => { const next = event.target.value; if (next === "" || (integer ? /^\d*$/.test(next) : /^\d*(?:\.\d*)?$/.test(next))) setDraft(next); }} onBlur={() => { focused.current = false; commit(draft); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); commit(draft); event.currentTarget.blur(); } }} onFocus={() => { focused.current = true; }} aria-valuemin={min} aria-valuemax={max} step={step} />;
}
