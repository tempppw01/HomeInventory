...
export function PrintStudio({ items, onClose }: { items: Item[]; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(printSettingsKey, JSON.stringify({ preset, customWidth, customHeight, columns, qrSize, horizontalMargin, verticalMargin, columnGap, rowGap, labelWidth, labelHeight, startRow, showLabelBorder, showLocation, showQuantity, showExpiry, showPurchaseDate, countMode, customCount }));
  }, [settingsLoaded, preset, customWidth, customHeight, columns, qrSize, horizontalMargin, verticalMargin, columnGap, rowGap, labelWidth, labelHeight, startRow, showLabelBorder, showLocation, showQuantity, showExpiry, showPurchaseDate, countMode, customCount]);
  ...
  const labelQrSize = Math.max(18, Math.min(scaledQrSize, Math.floor((availableQrWidth - qrGap) / 2)));
  const printCss = `
    @media print {
  ...
  return <div className="fixed inset-0 z-[80] flex flex-col bg-black/45 backdrop-blur-sm lg:flex-row">
    ...
    <aside className="print-controls z-10 w-full overflow-y-auto p-4 lg:h-screen lg:w-[360px]" style={{ background: "var(--surface-solid)" }}>
      ...
      <div className="mt-6 space-y-5">
        ...
        <div><div className="mb-2 text-xs font-bold muted">每件物品打印数量</div><div className="grid grid-cols-[1fr_110px] gap-3"><select className="input" value={countMode} onChange={(e) => setCountMode(e.target.value as PrintCountMode)}><option value="single">每件 1 个标签</option><option value="inventory">按库存数量打印</option><option value="custom">统一指定数量</option></select>{countMode === "custom" ? <input className="input" type="number" min="1" max="999" step="1" value={customCount} onChange={(e) => setCustomCount(Math.max(1, Math.min(999, Math.floor(Number(e.target.value) || 1))))} /> : <div className="input flex items-center text-xs muted">{countMode === "inventory" ? "按各自库存" : "1"}</div>}</div><p className="mb-0 mt-1.5 text-[11px] leading-5 muted">按库存时会使用每件物品的当前数量；指定数量会为每种物品打印相同份数。</p></div>
        <div><div className="mb-2 flex items-center gap-2 text-xs font-bold muted"><Settings2 size={14} />标签内容</div><div className="flex flex-wrap gap-2"><Toggle checked={showLabelBorder} onChange={setShowLabelBorder} label="二维码标签边框" /><Toggle checked={showLocation} onChange={setShowLocation} label="存放位置" /><Toggle checked={showQuantity} onChange={setShowQuantity} label="数量" /><Toggle checked={showPurchaseDate} onChange={setShowPurchaseDate} label="购买日期" /><Toggle checked={showExpiry} onChange={setShowExpiry} label="到期日" /></div></div>
      </div>
