  ...
  return <div className="fixed inset-0 z-[80] flex flex-col bg-black/45 backdrop-blur-sm lg:flex-row">
    ...
    <aside className="print-controls z-10 w-full overflow-y-auto p-4 lg:h-screen lg:w-[360px]" style={{ background: "var(--surface-solid)" }}>
      ...
      <div className="mt-6 space-y-5">
        ...
        <div><div className="mb-2 flex items-center gap-2 text-xs font-bold muted"><Settings2 size={14} />标签大小</div><div className="grid grid-cols-2 gap-3"><Control label="标签宽度 mm"><input className="input" type="number" min="20" max="200" step="0.1" value={labelWidth} onChange={(e) => setLabelWidth(Number(e.target.value))} /></Control><Control label="标签高度 mm"><input className="input" type="number" min="20" max="200" step="0.1" value={labelHeight} onChange={(e) => setLabelHeight(Number(e.target.value))} /></Control></div>{labelScale < 1 && <p className="mb-0 mt-2 text-[11px] leading-5 muted">标签较矮，二维码和文字已自动缩小以保留完整内容。</p>}</div>
