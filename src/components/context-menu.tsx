...
export function ContextMenu({ menu, items, onClose }: { menu: MenuState; items: ContextMenuItem[]; onClose: () => void }) {
  ...
  return <div role="menu" aria-label="快捷菜单" className="fixed z-[120] min-w-[208px] overflow-hidden rounded-2xl border p-1.5 shadow-2xl" style={{ left, top, background: "var(--surface-solid)", borderColor: "var(--border)" }} onPointerDown={(event) => event.stopPropagation()}>
