...
export function AiAssistantModal({ item, onClose, onApplied }: { item: Item; onClose: () => void; onApplied: (message: string) => void }) {
  ...
  const apply = async () => { if (!analysis) return; setApplying(true); try { const type = analysis.type || item.type; const patch = { name: analysis.name || item.name, category: analysis.category || item.category, type, unit: analysis.unit || item.unit, expiryDate: type === "DURABLE" ? "" : analysis.suggestedExpiryDate || item.expiryDate?.slice(0, 10) || "", notes: analysis.suggestedNotes || item.notes || "", aiSummary: analysis.summary || item.aiSummary || "", aiStorageAdvice: analysis.storageAdvice || item.aiStorageAdvice || "", aiUsageAdvice: analysis.usageAdvice || item.aiUsageAdvice || "", aiReplenishmentAdvice: analysis.replenishmentAdvice || item.aiReplenishmentAdvice || "" }; await jsonRequest(`/api/items/${item.id}`, { method: "PATCH", body: JSON.stringify(patch) }); onApplied("AI 关键信息已应用并显示在物品卡片"); onClose(); } catch (e) { setError(e instanceof Error ? e.message : "应用失败"); } finally { setApplying(false); } };

  return <div className="fixed inset-0 z-[75] flex items-end justify-center bg-black/40 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] p-5 shadow-2xl sm:rounded-[28px] sm:p-6" style={{ background: "var(--surface-solid)" }}><div className="flex items-start gap-3"><div className="grid size-11 place-items-center rounded-2xl" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}><Bot size={22} /></div><div className="min-w-0 flex-1"><h2 className="m-0 text-xl font-black">{item.name} · AI 助手</h2><p className="mb-0 mt-1 text-xs muted">{isMedicine ? "补齐说明书要点、适应症、常见用量与安全提醒。" : "识别纠错、保质期、存储方式和补货节奏。"}</p></div><button onClick={onClose} className="btn-ghost grid size-9 place-items-center p-0"><X size={17} /></button></div>
    {isMedicine && <p className="mt-4 rounded-xl px-3 py-2 text-xs leading-5 muted" style={{ background: "var(--surface-soft)" }}>AI 仅整理可确认的包装信息，不替代说明书、药师或医生的专业建议。</p>}
    <div className={`mt-5 grid gap-2 ${item.type === "CONSUMABLE" ? "grid-cols-3" : "grid-cols-2"}`}><Action icon={Sparkles} label={isMedicine ? "核对药品" : "智能识别"} disabled={loading} onClick={() => run("identify")} />{item.type === "CONSUMABLE" && <Action icon={CalendarClock} label={isMedicine ? "有效期" : "保质期"} disabled={loading} onClick={() => run("shelf_life")} />}<Action icon={Lightbulb} label={isMedicine ? "补全说明" : "全面分析"} disabled={loading} onClick={() => run("full")} /></div>
    ...
    {analysis && !loading && <div className="mt-5 space-y-3"><div className="rounded-2xl p-4" style={{ background: "var(--primary-soft)" }}><div className="text-sm font-bold">{analysis.summary || "分析完成"}</div>{typeof analysis.confidence === "number" && <div className="mt-1 text-xs muted">可信度 {Math.round(analysis.confidence * 100)}%</div>}</div><div className="grid gap-3 sm:grid-cols-2"><Result icon={PackageCheck} title={isMedicine ? "药品信息" : "识别结果"} text={[analysis.name, analysis.category, analysis.type === "CONSUMABLE" ? "消耗品" : analysis.type === "DURABLE" ? "耐用品" : null].filter(Boolean).join(" · ") || "没有建议修改"} />{(analysis.type || item.type) === "CONSUMABLE" && <Result icon={CalendarClock} title={isMedicine ? "有效期" : "保质期"} text={analysis.suggestedExpiryDate ? `建议到期日 ${analysis.suggestedExpiryDate}${analysis.expiryReason ? `，${analysis.expiryReason}` : ""}` : analysis.expiryReason || "无法可靠判断"} />}<Result icon={Warehouse} title={isMedicine ? "说明书与保存" : "存储建议"} text={analysis.storageAdvice || "暂无建议"} /><Result icon={Lightbulb} title={isMedicine ? "适应症、用量与提醒" : "使用与补货"} text={[analysis.usageAdvice, analysis.replenishmentAdvice].filter(Boolean).join("；") || "暂无建议"} /></div><button disabled={applying} onClick={apply} className="btn-primary flex w-full items-center justify-center gap-2"><Check size={17} />{applying ? "应用中…" : isMedicine ? "保存药品说明" : "应用可用建议"}</button></div>}
  </div></div>;
}
