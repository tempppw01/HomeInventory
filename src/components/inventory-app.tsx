...
import {
  Archive, Bath, Bell, Boxes, Check, ChevronRight, CircleAlert, CookingPot,
  Grid2X2, LayoutDashboard, MapPin, Minus, Moon,
  Package, Plus, Search, Settings, ShoppingBasket, Sofa, Sparkles,
...
type ItemDraft = {
  ...
  unit: string; price: string; purchaseDate: string; expiryDate: string; locationId: string; notes: string;
};

const emptyDraft: ItemDraft = {
  name: "", category: "日用", type: "DURABLE", quantity: 1, minQuantity: 0,
  unit: "件", price: "", purchaseDate: "", expiryDate: "", locationId: "", notes: "",
};
...
export function InventoryApp() {
  ...
  const [modal, setModal] = useState<"item" | "shopping" | "location" | null>(null);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("home-inventory-theme") as "light" | "dark" | null;
  ...
  const toggleTheme = () => {
    ...
    setTheme(next); localStorage.setItem("home-inventory-theme", next); document.documentElement.dataset.theme = next;
  };
  ...
  const filteredItems = useMemo(() => {
    ...
    return (data?.items ?? []).filter((item) =>
      ...
      (!term || [item.name, item.category, item.location?.name].some((value) => value?.toLowerCase().includes(term))),
    );
    ...
    <div className="min-h-screen md:grid md:grid-cols-[232px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 px-4 pb-8 pt-4 sm:px-6 md:px-8 md:py-7 xl:px-12">
        <header className="mb-7 flex items-center gap-3">
          ...
          <div className="desktop-only relative max-w-md flex-1">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 muted" size={18} />
            <input value={search} onChange={(e) => setSearch(e.target.value)} onFocus={() => setView("items")} className="input pl-11" placeholder="搜索物品、分类或位置…" />
          </div>
          <button onClick={toggleTheme} className="btn-ghost ml-auto grid size-11 place-items-center p-0" aria-label="切换主题">{theme === "light" ? <Moon size={19} /> : <Sun size={19} />}</button>
          <button className="btn-ghost relative grid size-11 place-items-center p-0" aria-label="提醒"><Bell size={19} />{(lowStock.length + expiring.length) > 0 && <span className="absolute right-2 top-2 size-2 rounded-full" style={{ background: "var(--danger)" }} />}</button>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .22 }}>
            {loading ? <LoadingView /> : view === "dashboard" ? (
              <DashboardView data={data!} lowStock={lowStock} expiring={expiring} pending={pendingShopping} totalValue={totalValue} onNavigate={setView} onEdit={openEdit} onConsume={consume} />
            ) : view === "items" ? (
              <ItemsView items={filteredItems} search={search} setSearch={setSearch} filter={typeFilter} setFilter={setTypeFilter} onEdit={openEdit} onConsume={consume} onDelete={removeItem} onAdd={() => setModal("item")} />
            ) : view === "shopping" ? (
            ...
            ) : <SettingsView theme={theme} onTheme={toggleTheme} />}
          </motion.div>
        </AnimatePresence>
      </main>
...
function DashboardView({ data, lowStock, expiring, pending, totalValue, onNavigate, onEdit, onConsume }: { data: DashboardData; lowStock: Item[]; expiring: Item[]; pending: ShoppingItem[]; totalValue: number; onNavigate: (v: View) => void; onEdit: (i: Item) => void; onConsume: (i: Item) => void }) {
  const stats = [
...
function ItemsView({ items, search, setSearch, filter, setFilter, onEdit, onConsume, onDelete, onAdd }: { items: Item[]; search: string; setSearch: (s: string) => void; filter: "ALL" | ItemType; setFilter: (f: "ALL" | ItemType) => void; onEdit: (i: Item) => void; onConsume: (i: Item) => void; onDelete: (i: Item) => void; onAdd: () => void }) {
  return <><PageTitle title="我的物品" text="随时知道家里有什么、放在哪里。" action={<button onClick={onAdd} className="btn-primary desktop-only flex items-center gap-2"><Plus size={18} />录入</button>} />
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center"><div className="relative flex-1 md:hidden"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 muted" size={18} /><input className="input pl-11" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索物品…" /></div><div className="flex gap-2 overflow-x-auto">{([ ["ALL", "全部"], ["DURABLE", "耐用品"], ["CONSUMABLE", "消耗品"] ] as const).map(([id, label]) => <button key={id} onClick={() => setFilter(id)} className="whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold transition" style={filter === id ? { background: "var(--primary)", color: "white" } : { background: "var(--surface-solid)", color: "var(--muted)", border: "1px solid var(--border)" }}>{label}</button>)}</div><span className="ml-auto text-sm muted">{items.length} 件</span></div>
    {items.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{items.map((item, index) => <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .025, .2) }}><ItemCard item={item} onEdit={() => onEdit(item)} onConsume={() => onConsume(item)} onDelete={() => onDelete(item)} /></motion.div>)}</div> : <div className="surface rounded-3xl py-16"><EmptyState icon={Search} title="没有找到物品" text="换个关键词或筛选条件试试" /></div>}
  </>;
}

function ItemCard({ item, onEdit, onConsume, onDelete, compact = false }: { item: Item; onEdit: () => void; onConsume: () => void; onDelete?: () => void; compact?: boolean }) {
  ...
  return <div onClick={onEdit} className={`group relative cursor-pointer overflow-hidden rounded-3xl border transition duration-200 hover:-translate-y-0.5 hover:shadow-lg ${compact ? "p-3" : "p-4"}`} style={{ background: "var(--surface-solid)", borderColor: "var(--border)" }}>
    <div className="flex items-start gap-3"><div className={`${compact ? "size-12 text-xl" : "size-14 text-2xl"} grid shrink-0 place-items-center rounded-2xl`} style={{ background: "var(--surface-soft)" }}>{emoji}</div><div className="min-w-0 flex-1"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h3 className="m-0 truncate text-sm font-extrabold sm:text-base">{item.name}</h3><p className="mb-0 mt-1 truncate text-xs muted">{item.location?.name || "未设置位置"} · {item.category}</p></div>{low && <span className="rounded-lg px-2 py-1 text-[10px] font-bold" style={{ background: "#ffe8e8", color: "#d54b57" }}>需补货</span>}</div>
      {!compact && <div className="mt-4 flex items-end justify-between"><div><span className="text-xl font-black">{item.quantity}</span><span className="ml-1 text-xs muted">{item.unit}</span></div>{item.type === "CONSUMABLE" && <button onClick={(event) => { event.stopPropagation(); onConsume(); }} className="btn-ghost flex items-center gap-1 px-2.5 py-1.5 text-xs"><Minus size={14} /> 用掉</button>}</div>}
    </div></div>
    {onDelete && <button onClick={(event) => { event.stopPropagation(); onDelete(); }} className="absolute right-3 top-3 hidden rounded-xl p-2 text-red-500 group-hover:block" style={{ background: "var(--surface-solid)" }}><Trash2 size={15} /></button>}
  </div>;
}
...
function SettingsView({ theme, onTheme }: { theme: string; onTheme: () => void }) {
  return <><PageTitle title="设置" text="调整归物的使用偏好。" /><div className="max-w-2xl space-y-4"><section className="surface rounded-3xl p-5"><h3 className="m-0 text-base">显示</h3><SettingRow icon={theme === "light" ? Sun : Moon} title="界面主题" text="跟随你的阅读习惯" action={<button className="btn-ghost text-sm" onClick={onTheme}>{theme === "light" ? "浅色" : "深色"}</button>} /></section><section className="surface rounded-3xl p-5"><h3 className="m-0 text-base">数据与部署</h3><SettingRow icon={Grid2X2} title="当前数据库" text="通过环境变量切换 SQLite / MySQL" action={<span className="rounded-xl px-3 py-1.5 text-xs font-bold" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>已连接</span>} /><SettingRow icon={Archive} title="归物" text="家庭物品管理系统 · v0.1.0" /></section></div></>;
}
...
function ItemModal({ locations, item, onClose, onSaved }: { locations: Location[]; item: Item | null; onClose: () => void; onSaved: () => void }) {
  const [draft, setDraft] = useState<ItemDraft>(() => item ? { name: item.name, category: item.category, type: item.type, quantity: item.quantity, minQuantity: item.minQuantity, unit: item.unit, price: item.price?.toString() ?? "", purchaseDate: item.purchaseDate?.slice(0, 10) ?? "", expiryDate: item.expiryDate?.slice(0, 10) ?? "", locationId: item.locationId ?? "", notes: item.notes ?? "" } : emptyDraft);
  ...
  return <Modal title={item ? "编辑物品" : "录入新物品"} subtitle="把常用信息记下来，今后查找更轻松。" onClose={onClose}><form onSubmit={submit} className="space-y-4"><div><label className="mb-1.5 block text-xs font-bold muted">物品名称 *</label><input autoFocus required className="input" value={draft.name} onChange={(e) => set("name", e.target.value)} placeholder="例如：洗衣液" /></div><div className="grid grid-cols-2 gap-3"><label className="cursor-pointer rounded-2xl border p-3 transition" style={draft.type === "DURABLE" ? { borderColor: "var(--primary)", background: "var(--primary-soft)" } : { borderColor: "var(--border)" }}><input type="radio" className="hidden" checked={draft.type === "DURABLE"} onChange={() => set("type", "DURABLE")} /><div className="text-sm font-bold">📦 耐用品</div><div className="mt-1 text-[11px] muted">家电、工具、衣物</div></label><label className="cursor-pointer rounded-2xl border p-3 transition" style={draft.type === "CONSUMABLE" ? { borderColor: "var(--primary)", background: "var(--primary-soft)" } : { borderColor: "var(--border)" }}><input type="radio" className="hidden" checked={draft.type === "CONSUMABLE"} onChange={() => set("type", "CONSUMABLE")} /><div className="text-sm font-bold">🧴 消耗品</div><div className="mt-1 text-[11px] muted">食品、清洁、日用</div></label></div><div className="grid grid-cols-2 gap-3"><Field label="分类"><select className="input" value={draft.category} onChange={(e) => set("category", e.target.value)}>{categories.map((c) => <option key={c}>{c}</option>)}</select></Field><Field label="存放位置"><select className="input" value={draft.locationId} onChange={(e) => set("locationId", e.target.value)}><option value="">未设置</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></Field></div><div className="grid grid-cols-[1fr_1fr_1fr] gap-3"><Field label="数量"><input required type="number" min="0" step="0.1" className="input" value={draft.quantity} onChange={(e) => set("quantity", Number(e.target.value))} /></Field><Field label="单位"><select className="input" value={draft.unit} onChange={(e) => set("unit", e.target.value)}>{units.map((u) => <option key={u}>{u}</option>)}</select></Field>{draft.type === "CONSUMABLE" ? <Field label="提醒阈值"><input type="number" min="0" step="0.1" className="input" value={draft.minQuantity} onChange={(e) => set("minQuantity", Number(e.target.value))} /></Field> : <Field label="单价"><input type="number" min="0" step="0.01" className="input" value={draft.price} onChange={(e) => set("price", e.target.value)} placeholder="¥" /></Field>}</div><div className="grid grid-cols-2 gap-3"><Field label="购入日期"><input type="date" className="input" value={draft.purchaseDate} onChange={(e) => set("purchaseDate", e.target.value)} /></Field><Field label="到期日期"><input type="date" className="input" value={draft.expiryDate} onChange={(e) => set("expiryDate", e.target.value)} /></Field></div><Field label="备注"><textarea className="input min-h-20 resize-none" value={draft.notes} onChange={(e) => set("notes", e.target.value)} placeholder="规格、保修、使用提示…" /></Field>{error && <p className="m-0 text-sm text-red-500">{error}</p>}<div className="flex gap-3 pt-2"><button type="button" onClick={onClose} className="btn-ghost flex-1">取消</button><button disabled={saving} className="btn-primary flex-1 disabled:opacity-60">{saving ? "保存中…" : item ? "保存修改" : "确认录入"}</button></div></form></Modal>;
}
