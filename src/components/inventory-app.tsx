  ...
  return <div onClick={onEdit} className={`group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-3xl border transition duration-200 hover:-translate-y-0.5 hover:shadow-lg ${compact ? "p-3" : "min-h-40 p-4"}`} style={{ background: "var(--surface-solid)", borderColor: selected ? "var(--primary)" : "var(--border)", boxShadow: selected ? "0 0 0 2px color-mix(in srgb, var(--primary) 18%, transparent)" : undefined }}>
    ...
    <div className="flex flex-1 items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h3 className="m-0 truncate text-sm font-extrabold sm:text-base">{item.name}</h3><p className="mb-0 mt-1 truncate text-xs muted">{item.location?.name || "未设置位置"} · {item.category}</p><p className="mb-0 mt-1 truncate text-[10px] font-semibold muted">{item.itemCode || item.id}</p></div>{low && <span className="rounded-lg px-2 py-1 text-[10px] font-bold" style={{ background: "#ffe8e8", color: "#d54b57" }}>需补货</span>}</div>
...
type OssForm = { configured: boolean; managedByEnvironment: boolean; region: string; endpoint: string; bucket: string; directory: string; accessKeyId: string; accessKeySecretConfigured: boolean; publicBaseUrl: string; accessKeySecret: string };
const emptyOssForm: OssForm = { configured: false, managedByEnvironment: false, region: "", endpoint: "", bucket: "", directory: "home-inventory", accessKeyId: "", accessKeySecretConfigured: false, publicBaseUrl: "", accessKeySecret: "" };
...
function OssSettings({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <details className="surface group rounded-3xl p-5"><summary className="flex cursor-pointer list-none items-center gap-3 [&::-webkit-details-marker]:hidden"><div className="grid size-11 shrink-0 place-items-center rounded-2xl" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}><Cloud size={20} /></div><div className="min-w-0 flex-1"><h3 className="m-0 text-sm font-black">OSS 图片存储</h3><p className="mb-0 mt-1 truncate text-xs muted">{form.configured ? `${form.bucket}/${form.directory}` : "阿里云 OSS 与兼容对象存储"}</p></div><span className="rounded-full px-2.5 py-1 text-[10px] font-bold" style={{ background: form.configured ? "#e0f7ef" : "var(--surface-soft)", color: form.configured ? "var(--success)" : "var(--muted)" }}>{loading ? "检测中" : form.configured ? "已配置" : "未配置"}</span><ChevronDown size={17} className="muted transition-transform group-open:rotate-180" /></summary>
    {!loading && <form onSubmit={save} className="mt-5 space-y-3 border-t pt-5" style={{ borderColor: "var(--border)" }}><div className="grid gap-3 sm:grid-cols-2"><Field label="Region"><input required disabled={form.managedByEnvironment} className="input" value={form.region} onChange={(e) => set("region", e.target.value)} placeholder="oss-cn-hangzhou" /></Field><Field label="Bucket"><input required disabled={form.managedByEnvironment} className="input" value={form.bucket} onChange={(e) => set("bucket", e.target.value)} placeholder="home-inventory" /></Field></div><Field label="存储目录"><input required disabled={form.managedByEnvironment} className="input" value={form.directory} onChange={(e) => set("directory", e.target.value)} placeholder="home-inventory" /><span className="mt-1.5 block text-[11px] leading-5 muted">图片会写入 Bucket 的此目录，例如 home-inventory/items/日期，不再占用根目录。</span></Field><Field label="Endpoint（可选）"><input disabled={form.managedByEnvironment} className="input" value={form.endpoint} onChange={(e) => set("endpoint", e.target.value)} placeholder="https://oss-cn-hangzhou.aliyuncs.com" /></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="AccessKey ID"><input required disabled={form.managedByEnvironment} className="input" value={form.accessKeyId} onChange={(e) => set("accessKeyId", e.target.value)} autoComplete="off" /></Field><Field label="AccessKey Secret"><input disabled={form.managedByEnvironment} className="input" type="password" value={form.accessKeySecret} onChange={(e) => set("accessKeySecret", e.target.value)} placeholder={form.accessKeySecretConfigured ? "已保存，留空则不修改" : "首次配置必填"} autoComplete="new-password" /></Field></div><Field label="公开访问域名（可选）"><input disabled={form.managedByEnvironment} className="input" value={form.publicBaseUrl} onChange={(e) => set("publicBaseUrl", e.target.value)} placeholder="https://img.example.com" /></Field><div className="flex items-center justify-between gap-3 pt-1"><p className="m-0 text-[11px] leading-5 muted">Secret 仅保存在服务端数据库，不会返回浏览器。目录也可用 OSS_DIRECTORY 环境变量管理。</p>{form.managedByEnvironment ? <span className="whitespace-nowrap text-xs font-bold muted">环境变量托管</span> : <button disabled={saving} className="btn-primary whitespace-nowrap px-4 py-2 text-sm">{saving ? "保存中…" : "保存 OSS"}</button>}</div></form>}
  </details>;
}
