...
import {
  Archive, Bath, Bell, Boxes, Check, ChevronRight, CircleAlert, Cloud, CookingPot,
  Grid2X2, ImagePlus, LayoutDashboard, MapPin, Minus, Monitor, Moon,
  Package, Plus, QrCode, Search, Settings, ShoppingBasket, Sofa, Sparkles,
  Sun, Trash2, Upload, Warehouse, X, Zap,
} from "lucide-react";
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .22 }}>
            {loading ? <LoadingView /> : view === "dashboard" ? (
              <DashboardView data={data!} lowStock={lowStock} expiring={expiring} pending={pendingShopping} totalValue={totalValue} onNavigate={setView} onEdit={openEdit} onConsume={consume} onQr={setQrItem} />
            ) : view === "items" ? (
              <ItemsView allItems={data!.items} items={filteredItems} search={search} setSearch={setSearch} filter={typeFilter} setFilter={setTypeFilter} onEdit={openEdit} onConsume={consume} onDelete={removeItem} onQr={setQrItem} />
            ) : view === "shopping" ? (
...
function DashboardView({ data, lowStock, expiring, pending, totalValue, onNavigate, onEdit, onConsume, onQr }: { data: DashboardData; lowStock: Item[]; expiring: Item[]; pending: ShoppingItem[]; totalValue: number; onNavigate: (v: View) => void; onEdit: (i: Item) => void; onConsume: (i: Item) => void; onQr: (i: Item) => void }) {
  const stats = [
...
function ItemCard({ item, onEdit, onConsume, onDelete, onQr, compact = false }: { item: Item; onEdit: () => void; onConsume: () => void; onDelete?: () => void; onQr: () => void; compact?: boolean }) {
  ...
  return <div onClick={onEdit} className={`group relative cursor-pointer overflow-hidden rounded-3xl border transition duration-200 hover:-translate-y-0.5 hover:shadow-lg ${compact ? "p-3" : "p-4"}`} style={{ background: "var(--surface-solid)", borderColor: "var(--border)" }}>
    <div className="flex items-start gap-3"><div className={`${compact ? "size-12 text-xl" : "size-14 text-2xl"} grid shrink-0 place-items-center rounded-2xl bg-cover bg-center`} style={item.imageUrl ? { backgroundImage: `url(${item.imageUrl})` } : { background: "var(--surface-soft)" }}>{!item.imageUrl && emoji}</div><div className="min-w-0 flex-1"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h3 className="m-0 truncate text-sm font-extrabold sm:text-base">{item.name}</h3><p className="mb-0 mt-1 truncate text-xs muted">{item.location?.name || "未设置位置"} · {item.category}</p><p className="mb-0 mt-1 truncate text-[10px] font-semibold muted">{item.itemCode || item.id}</p></div>{low && <span className="rounded-lg px-2 py-1 text-[10px] font-bold" style={{ background: "#ffe8e8", color: "#d54b57" }}>需补货</span>}</div>
      {!compact && <div className="mt-4 flex items-end justify-between gap-2"><div><span className="text-xl font-black">{item.quantity}</span><span className="ml-1 text-xs muted">{item.unit}</span></div><div className="flex gap-2"><button onClick={(event) => { event.stopPropagation(); onQr(); }} className="btn-ghost grid size-8 place-items-center p-0" aria-label="显示二维码"><QrCode size={15} /></button>{item.type === "CONSUMABLE" && <button onClick={(event) => { event.stopPropagation(); onConsume(); }} className="btn-ghost flex items-center gap-1 px-2.5 py-1.5 text-xs"><Minus size={14} /> 用掉</button>}</div></div>}
    </div></div>
    {compact && <button onClick={(event) => { event.stopPropagation(); onQr(); }} className="absolute bottom-2 right-2 grid size-7 place-items-center rounded-lg opacity-60 transition hover:opacity-100" style={{ background: "var(--surface-soft)" }} aria-label="显示二维码"><QrCode size={13} /></button>}
...
function SettingsView({ theme, onTheme, onToast }: { theme: ThemeMode; onTheme: (mode: ThemeMode) => void; onToast: (message: string) => void }) {
  return <><PageTitle title="设置" text="调整归物的显示、存储和部署偏好。" /><div className="max-w-3xl space-y-4">
    ...
    <section className="surface rounded-3xl p-5"><h3 className="m-0 text-base">数据与部署</h3><SettingRow icon={Grid2X2} title="当前数据库" text="通过环境变量切换 SQLite / MySQL" action={<span className="rounded-xl px-3 py-1.5 text-xs font-bold" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>已连接</span>} /><SettingRow icon={Archive} title="归物" text="家庭物品管理系统 · v0.2.0" /></section>
  </div></>;
}
...
function QrModal({ item, onClose }: { item: Item; onClose: () => void }) {
  ...
  return <Modal title="物品二维码" subtitle="扫码即可快速查看物品信息" onClose={onClose}><div className="grid place-items-center"><div className="rounded-3xl bg-white p-5"><QRCodeSVG value={url} size={220} level="M" includeMargin /></div><div className="mt-4 text-center"><div className="text-lg font-black">{item.name}</div><div className="mt-1 font-mono text-xs muted">{item.itemCode || item.id}</div></div><a href={url} target="_blank" rel="noreferrer" className="btn-ghost mt-5 text-sm">打开物品详情</a></div></Modal>;
}
...
function ItemModal({ locations, item, onClose, onSaved }: { locations: Location[]; item: Item | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={item ? "编辑物品" : "录入新物品"} subtitle={item?.itemCode || "编号将在保存后自动生成"} onClose={onClose}><form onSubmit={submit} className="space-y-4">
    <label className="group flex cursor-pointer items-center gap-4 rounded-2xl border border-dashed p-3 transition hover:border-[var(--primary)]" style={{ borderColor: "var(--border)" }}><div className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-2xl bg-cover bg-center" style={draft.imageUrl ? { backgroundImage: `url(${draft.imageUrl})` } : { background: "var(--surface-soft)" }}>{!draft.imageUrl && <ImagePlus className="muted" size={24} />}</div><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-bold"><Upload size={16} />{uploading ? "正在上传…" : draft.imageUrl ? "更换物品图片" : "上传物品图片"}</div><div className="mt-1 text-xs muted">图片保存到 OSS，支持 JPG / PNG / WebP，最大 5MB</div></div><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" disabled={uploading} onChange={(e) => uploadImage(e.target.files?.[0])} /></label>
    <div><label className="mb-1.5 block text-xs font-bold muted">物品名称 *</label><input autoFocus required className="input" value={draft.name} onChange={(e) => set("name", e.target.value)} placeholder="例如：洗衣液" /></div>
    <div className="grid grid-cols-2 gap-3"><label className="cursor-pointer rounded-2xl border p-3 transition" style={draft.type === "DURABLE" ? { borderColor: "var(--primary)", background: "var(--primary-soft)" } : { borderColor: "var(--border)" }}><input type="radio" className="hidden" checked={draft.type === "DURABLE"} onChange={() => set("type", "DURABLE")} /><div className="text-sm font-bold">📦 耐用品</div><div className="mt-1 text-[11px] muted">家电、工具、衣物</div></label><label className="cursor-pointer rounded-2xl border p-3 transition" style={draft.type === "CONSUMABLE" ? { borderColor: "var(--primary)", background: "var(--primary-soft)" } : { borderColor: "var(--border)" }}><input type="radio" className="hidden" checked={draft.type === "CONSUMABLE"} onChange={() => set("type", "CONSUMABLE")} /><div className="text-sm font-bold">🧴 消耗品</div><div className="mt-1 text-[11px] muted">食品、清洁、日用</div></label></div>
