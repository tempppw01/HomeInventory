...
function ItemCard({ item, onEdit, onConsume, onRemainingChange, onDelete, onQr, onAi, onCopy, onSelect, onSelectionPointerDown, onSelectionPointerEnter, selected = false, compact = false }: { item: Item; onEdit: () => void; onConsume: () => void; onRemainingChange?: (remainingPercent: number) => void; onDelete?: () => void; onQr: () => void; onAi: () => void; onCopy?: () => void; onSelect?: () => void; onSelectionPointerDown?: () => void; onSelectionPointerEnter?: () => void; selected?: boolean; compact?: boolean }) {
  ...
  return <><div onClick={onEdit} onPointerDown={(event) => { if (!(event.target as HTMLElement).closest("button")) onSelectionPointerDown?.(); }} onPointerEnter={() => onSelectionPointerEnter?.()} onContextMenu={context.onContextMenu} className={`group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-xl border transition duration-200 hover:border-[color-mix(in_srgb,var(--primary)_38%,var(--border))] ${compact ? "p-3" : "min-h-40 p-4"}`} style={{ background: "var(--surface-solid)", borderColor: selected ? "var(--primary)" : "var(--border)", boxShadow: selected ? "0 0 0 2px color-mix(in srgb, var(--primary) 18%, transparent)" : undefined }}>
    ...
    <div className={`mt-auto flex h-8 items-center gap-2 ${compact ? "pt-2" : "pt-4"}`}><div className="flex min-w-0 flex-1 items-center">{!compact && <><span className="text-xl font-black leading-none">{item.quantity}</span><span className="ml-1 text-xs muted">{item.unit}</span></>}</div><div className="flex h-8 shrink-0 items-center gap-1.5"><button onClick={(event) => { event.stopPropagation(); onAi(); }} className="btn-ghost grid h-8 min-h-8 w-8 place-items-center p-0" aria-label="AI 物品助手" title="AI 物品助手"><Bot size={15} /></button><button onClick={(event) => { event.stopPropagation(); onQr(); }} className="btn-ghost grid h-8 min-h-8 w-8 place-items-center p-0" aria-label="显示二维码" title="显示二维码"><QrCode size={15} /></button>{!compact && item.type === "CONSUMABLE" && <button onClick={(event) => { event.stopPropagation(); onConsume(); }} className="btn-ghost flex h-8 min-h-8 items-center gap-1 px-2.5 py-0 text-xs"><Minus size={14} /> 用掉</button>}{!compact && onDelete && <button onClick={(event) => { event.stopPropagation(); onDelete(); }} className="btn-ghost hidden h-8 min-h-8 w-8 place-items-center p-0 text-red-500 group-hover:grid" aria-label="删除物品"><Trash2 size={14} /></button>}</div></div>
