...
function AboutView() {
  return <><PageTitle title="关于归物" text="轻量、清楚、真正适合家庭日常使用。" /><div className="max-w-3xl space-y-4"><section className="surface rounded-3xl p-6"><div className="flex items-center gap-4"><div className="grid size-14 place-items-center rounded-3xl text-white" style={{ background: "var(--primary)" }}><Archive size={26} /></div><div><h2 className="m-0 text-xl font-black">归物 HomeInventory</h2><p className="mb-0 mt-1 text-sm muted">版本 {APP_VERSION} · MVP 迭代起点</p></div></div><p className="mb-0 mt-5 text-sm leading-7 muted">归物帮助家庭记录物品、消耗品、保质期、采购与价格。功能设计遵循“少一步操作、少一个干扰”的原则，复杂能力放在需要时再展开。</p></section><section className="surface rounded-3xl p-6"><h3 className="m-0 text-base">{APP_VERSION} 更新内容</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><AboutFeature icon={Package} title="开箱示范" text="空库自动准备少量示范数据，已有家庭数据绝不覆盖。" /><AboutFeature icon={AlertTriangle} title="保质期预警" text="区分已过期和即将到期，食品提醒更明确。" /><AboutFeature icon={WalletCards} title="消费概览" text="保留购买价格流水与近 6 月均值。" /></div></section></div></>;
}
