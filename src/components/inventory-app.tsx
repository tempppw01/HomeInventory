...
import {
  AlertTriangle, Archive, Bath, Bell, Bot, Boxes, CalendarDays, Check, CheckCircle2, CheckSquare, ChevronDown, ChevronRight, CircleAlert, Cloud, CookingPot,
...
const commonItemTemplates = [
  ...
  { keywords: ["牛奶", "酸奶", "鸡蛋", "蔬菜", "水果", "豆腐"], category: "食品", type: "CONSUMABLE" as ItemType, locationKeywords: ["冷藏", "冰箱"] },
  { keywords: ["可乐", "矿泉水", "饮料", "咖啡", "茶"], category: "饮品", type: "CONSUMABLE" as ItemType, locationKeywords: ["冷藏", "餐边", "厨房"] },
  { keywords: ["洗衣液", "洗洁精", "消毒液", "清洁剂", "纸巾", "湿巾"], category: "清洁", type: "CONSUMABLE" as ItemType, locationKeywords: ["洗衣", "清洁", "浴室", "卫生间"] },
  { keywords: ["退烧药", "感冒药", "创可贴", "药", "口罩"], category: "医药", type: "CONSUMABLE" as ItemType, locationKeywords: ["药箱", "医药"] },
] as const;
...
function locationPath(location: Location | null | undefined, locations: Location[]) {
  if (!location) return "未设置位置";
  const names = [location.name];
  let parentId = location.parentId;
  let guard = 0;
  while (parentId && guard < 8) {
    const parent = locations.find((entry) => entry.id === parentId);
    if (!parent) break;
...
function estimatedUseDays(item: Item) {
  if (item.type !== "CONSUMABLE" || isLiquidConsumable(item) || !item.consumeRate || item.consumeRate < 0.1 || item.quantity <= 0) return null;
  return Math.max(1, Math.ceil(item.quantity / item.consumeRate));
}

function findCommonItemTemplate(name: string) {
  const normalized = normalizedItemName(name);
...
export function InventoryApp() {
  ...
  useEffect(() => {
    const key = "home-inventory-auto-backup-check";
    if (localStorage.getItem(key) === new Date().toISOString().slice(0, 10)) return;
    fetch("/api/backup/auto", { method: "POST" }).then(() => localStorage.setItem(key, new Date().toISOString().slice(0, 10))).catch(() => undefined);
  }, []);
  ...
  const toggleTask = async (task: HouseholdTask) => { try { setToastAction(null); const previousDueAt = task.dueAt; await request("/api/tasks", { method: "PATCH", body: JSON.stringify({ id: task.id, action: "toggle" }) }); setToast(task.recurrence === "NONE" ? "任务已完成" : "已完成，本次任务已顺延"); setToastAction({ label: "撤销", onClick: async () => { try { await request("/api/tasks", { method: "PATCH", body: JSON.stringify({ id: task.id, action: "undo", previousDueAt }) }); setToastAction(null); setToast("任务已恢复为待办"); await refresh(); } catch (error) { setToast(error instanceof Error ? error.message : "撤销失败"); } } }); await refresh(); } catch (error) { setToast(error instanceof Error ? error.message : "更新任务失败"); } };
  ...
  const duplicateItem = async (item: Item) => {
    try {
      await request<Item>("/api/items", { method: "POST", body: JSON.stringify({
        name: `${item.name}（副本）`, category: item.category, type: item.type,
        quantity: isCountUnit(item.unit) ? 1 : item.quantity, minQuantity: item.minQuantity,
        remainingPercent: item.remainingPercent, unit: item.unit, price: item.price,
        purchaseDate: item.purchaseDate, expiryDate: item.expiryDate, imageUrl: item.imageUrl,
        locationId: item.locationId, notes: item.notes, aiSummary: item.aiSummary,
        aiStorageAdvice: item.aiStorageAdvice, aiUsageAdvice: item.aiUsageAdvice,
        aiReplenishmentAdvice: item.aiReplenishmentAdvice,
      }) });
      setToast(`${item.name} 已复制为新物品`);
      await refresh();
    } catch (error) { setToast(error instanceof Error ? error.message : "复制物品失败"); }
  };
  ...
  const toggleFavoriteShopping = async (item: ShoppingItem) => {
    await request(`/api/shopping/${item.id}`, { method: "PATCH", body: JSON.stringify({ isFavorite: !item.isFavorite }) });
    setToast(item.isFavorite ? "已取消常买标记" : "已加入常买清单");
    await refresh();
  };
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .22 }}>
            {loading ? <LoadingView /> : view === "dashboard" ? (
              ...
              <ItemsView allItems={data!.items.filter((item) => item.quantity > 0)} locations={data!.locations} items={filteredItems} shopping={data!.shopping} search={search} setSearch={setSearch} filter={typeFilter} setFilter={changeTypeFilter} categoryFilter={categoryFilter} setCategoryFilter={changeCategoryFilter} onEdit={openEdit} onOpenDetail={openDetail} onImageUpload={uploadItemImage} onDuplicate={duplicateItem} onConsume={consume} onRemainingChange={updateRemaining} onDelete={removeItem} onQr={setQrItem} onAi={setAiItem} onPrint={setPrintItems} onToast={setToast} onCopy={(item) => copyText(item.itemCode || item.id, "物品编号已复制")} onToggleShopping={toggleShopping} onFavoriteShopping={toggleFavoriteShopping} onAddShopping={() => setModal("shopping")} onDeleteShopping={async (id) => { await request(`/api/shopping/${id}`, { method: "DELETE" }); await refresh(); }} />
            ) : view === "locations" ? (
...
function HomeInsightsCompact({ data }: { data: DashboardData }) {
  if (data.finance.currentMonthTotal <= 0 && data.finance.averageMonthly <= 0 && !data.finance.recent[0] && data.finance.byCategory.length === 0 && data.consumption.length === 0) return null;
  return <section className="surface p-4 sm:p-5"><h2 className="m-0 text-base font-black">家庭状态</h2><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-2xl p-3" style={{ background: "var(--surface-soft)" }}><div className="flex items-center gap-1.5 text-[11px] font-bold muted"><WalletCards size={13} />本月消费</div><div className="mt-2 text-lg font-black">¥{data.finance.currentMonthTotal.toFixed(0)}</div></div><div className="rounded-2xl p-3" style={{ background: "var(--surface-soft)" }}><div className="text-[11px] font-bold muted">近 6 月均值</div><div className="mt-2 text-lg font-black">¥{data.finance.averageMonthly.toFixed(0)}</div></div></div>{data.finance.byCategory.length > 0 && <div className="mt-4 border-t pt-3" style={{ borderColor: "var(--border)" }}><div className="mb-2 text-[11px] font-bold muted">本月消费分类</div><div className="flex flex-wrap gap-2">{data.finance.byCategory.map((entry) => <span key={entry.category} className="rounded-lg px-2 py-1 text-[11px] font-bold" style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>{entry.category} ¥{entry.total.toFixed(0)}</span>)}</div></div>}{data.consumption.length > 0 && <div className="mt-4 border-t pt-3" style={{ borderColor: "var(--border)" }}><div className="mb-2 text-[11px] font-bold muted">最近消耗较多</div><div className="space-y-1.5">{data.consumption.slice(0, 3).map((entry) => <div key={entry.itemName} className="flex items-center justify-between gap-2 text-xs"><span className="min-w-0 truncate">{entry.itemName}</span><span className="shrink-0 font-bold muted">{entry.count} 次</span></div>)}</div></div>}{data.finance.recent[0] && <div className="mt-3 truncate text-[11px] muted">最近：{data.finance.recent[0].itemName} ¥{data.finance.recent[0].totalPrice.toFixed(2)}</div>}</section>;
}
...
function ItemsView({ allItems, locations, items, shopping, search, setSearch, filter, setFilter, categoryFilter, setCategoryFilter, onEdit, onOpenDetail, onImageUpload, onDuplicate, onConsume, onRemainingChange, onDelete, onQr, onAi, onPrint, onToast, onCopy, onToggleShopping, onFavoriteShopping, onAddShopping, onDeleteShopping }: { allItems: Item[]; locations: Location[]; items: Item[]; shopping: ShoppingItem[]; search: string; setSearch: (s: string) => void; filter: ItemQuickFilter; setFilter: (f: ItemQuickFilter) => void; categoryFilter: string; setCategoryFilter: (f: string) => void; onEdit: (i: Item) => void; onOpenDetail: (i: Item) => void; onImageUpload: (i: Item, file?: File) => void; onDuplicate: (i: Item) => void; onConsume: (i: Item) => void; onRemainingChange: (item: Item, remainingPercent: number) => void; onDelete: (i: Item) => void; onQr: (i: Item) => void; onAi: (i: Item) => void; onPrint: (items: Item[]) => void; onToast: (message: string) => void; onCopy: (i: Item) => void; onToggleShopping: (item: ShoppingItem) => void; onFavoriteShopping: (item: ShoppingItem) => void; onAddShopping: () => void; onDeleteShopping: (id: string) => void }) {
...
function ItemCard({ item, onEdit, onOpenDetail, onImageUpload, onDuplicate, onConsume, onRemainingChange, onDelete, onQr, onAi, onCopy, onSelect, selected = false, compact = false }: { item: Item; onEdit: () => void; onOpenDetail?: () => void; onImageUpload?: (file: File) => void; onDuplicate?: () => void; onConsume: () => void; onRemainingChange?: (remainingPercent: number) => void; onDelete?: () => void; onQr: () => void; onAi: () => void; onCopy?: () => void; onSelect?: () => void; selected?: boolean; compact?: boolean }) {
  ...
  const expectedDays = estimatedUseDays(item);
  const ai = itemAiHighlights(item);
  ...
  return <><div onContextMenu={context.onContextMenu} className={`group relative flex h-full min-w-0 flex-col overflow-hidden rounded-xl border transition duration-200 hover:border-[color-mix(in_srgb,var(--primary)_38%,var(--border))] ${compact ? "p-3" : "min-h-40 p-4"}`} style={{ background: "var(--surface-solid)", borderColor: selected ? "var(--primary)" : "var(--border)", boxShadow: selected ? "0 0 0 2px color-mix(in srgb, var(--primary) 18%, transparent)" : undefined }}>
    <div className="flex min-w-0 flex-1 items-start gap-3">
      <label className={`${compact ? "size-12 text-xl" : "size-14 text-2xl"} relative grid shrink-0 cursor-pointer place-items-center rounded-2xl bg-cover bg-center`} style={item.imageUrl ? { backgroundImage: `url(${item.imageUrl})` } : { background: "var(--surface-soft)" }} title="点击上传物品照片">{!item.imageUrl && emoji}{onImageUpload && <><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) onImageUpload(file); event.currentTarget.value = ""; }} /><span className="pointer-events-none absolute inset-0 grid place-items-center rounded-2xl bg-black/35 text-white opacity-0 transition group-hover:opacity-100"><ImagePlus size={16} /></span></>}</label>
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-2"><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-1.5"><button type="button" onClick={onOpenDetail} className="min-w-0 truncate text-left text-sm font-extrabold hover:underline sm:text-base">{item.name}</button><span className="shrink-0 text-xs font-bold" style={{ color: "var(--primary)" }}>{item.quantity}{item.unit}</span><span className="shrink-0 rounded-md px-1.5 py-0.5 text-[10px] font-bold" style={{ background: item.type === "CONSUMABLE" ? "var(--primary-soft)" : "var(--surface-soft)", color: item.type === "CONSUMABLE" ? "var(--primary)" : "var(--muted)" }}>{item.type === "CONSUMABLE" ? "消耗品" : "耐用品"}</span></div><p className="mb-0 mt-1 truncate text-xs muted">{item.location?.name || "未设置位置"} · {item.category}</p>{item.type === "CONSUMABLE" && item.purchaseDate && <p className="mb-0 mt-1 truncate text-[10px] font-semibold muted">购买日期：{new Date(item.purchaseDate).toLocaleDateString("zh-CN")}</p>}<p className="mb-0 mt-1 truncate text-[10px] font-semibold muted">{item.itemCode || item.id}</p></div>{low && <span className="rounded-lg px-2 py-1 text-[10px] font-bold" style={{ background: "#ffe8e8", color: "#d54b57" }}>需补货</span>}</div>
        {!compact && (dailyCost || expectedDays) && <div className="mt-3 flex items-center justify-between gap-2 rounded-xl px-2.5 py-2 text-[11px]" style={{ background: "var(--surface-soft)" }}>{dailyCost ? <span className="font-bold">日均 ¥{dailyCost.cost.toFixed(dailyCost.cost >= 10 ? 0 : 2)}</span> : <span className="muted">消耗趋势</span>}{expectedDays ? <span className="muted">预计还能用 {expectedDays} 天</span> : dailyCost ? <span className="muted">已使用 {dailyCost.days} 天</span> : null}</div>}
...
function ShoppingRow({ item, onToggle, onFavorite, onDelete, compact = false }: { item: ShoppingItem; onToggle: () => void; onFavorite?: () => void; onDelete: () => void; compact?: boolean }) {
...
function TasksView({ tasks, onAdd, onToggle, onDelete }: { tasks: HouseholdTask[]; onAdd: () => void; onToggle: (task: HouseholdTask) => void; onDelete: (task: HouseholdTask) => void }) {
  ...
  return <><PageTitle title="家庭任务" text="把清洁、保养和日常小事，安排成不会忘记的节奏。" action={<button onClick={onAdd} className="btn-primary flex items-center gap-2"><Plus size={18} /><span className="desktop-only">安排任务</span></button>} />
    ...
    <section className="surface p-4 sm:p-5"><div className="space-y-1">{tasks.map((task) => { const due = new Date(task.dueAt); const overdue = due.getTime() < today.getTime(); return <div key={task.id} className="group flex items-center gap-3 border-b py-3 last:border-b-0" style={{ borderColor: "var(--border)" }}><button onClick={() => onToggle(task)} className="btn-ghost flex h-9 shrink-0 items-center gap-1.5 rounded-xl px-2.5 text-xs font-bold" aria-label={`完成任务：${task.title}`} title="标记为完成" style={{ color: overdue ? "var(--danger)" : "var(--primary)" }}><CheckCircle2 size={17} /><span className="hidden sm:inline">完成</span></button><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold">{task.title}</div><div className="mt-0.5 flex flex-wrap items-center gap-2 text-xs muted"><span>{overdue ? "已到期" : `到期 ${due.toLocaleDateString("zh-CN", { month: "numeric", day: "numeric" })}`}</span><span>·</span><span>{recurrenceLabels[task.recurrence]}</span>{task.description && <><span>·</span><span className="truncate">{task.description}</span></>}</div></div><button onClick={() => onDelete(task)} className="icon-action opacity-0 transition group-hover:opacity-100" aria-label="删除任务"><Trash2 size={15} /></button></div>; })}{tasks.length === 0 && <EmptyState icon={ListTodo} title="还没有安排任务" text="从一次清洁或保养开始，让家务变得有节奏。" />}</div></section>
  </>;
}
...
function SettingsView({ onToast, onAbout, onAudit, onRecycle, density, motionMode, onDensityChange, onMotionChange }: { onToast: (message: string) => void; onAbout: () => void; onAudit: () => void; onRecycle: () => void; density: "comfortable" | "compact"; motionMode: "standard" | "reduced"; onDensityChange: (value: "comfortable" | "compact") => void; onMotionChange: (value: "standard" | "reduced") => void }) {
  ...
  return <><PageTitle title="设置" text="按类别展开需要修改的设置，保持页面简洁。" action={<div className="flex items-center gap-2"><button onClick={onAudit} className="btn-ghost grid size-10 shrink-0 place-items-center p-0 sm:flex sm:h-auto sm:w-auto sm:gap-2 sm:px-3" aria-label="进入家庭盘点" title="家庭盘点"><ClipboardCheck size={16} /><span className="hidden sm:inline">盘点</span></button><button onClick={onRecycle} className="btn-ghost grid size-10 shrink-0 place-items-center p-0 sm:flex sm:h-auto sm:w-auto sm:gap-2 sm:px-3" aria-label="打开回收站" title="回收站"><Trash2 size={16} /><span className="hidden sm:inline">回收站</span></button></div>} /><div className="grid max-w-5xl items-start gap-4 lg:grid-cols-2">
    ...
    <HouseholdPreferences onToast={onToast} />
...
function ItemModal({ locations, allItems, item, onClose, onSaved }: { locations: Location[]; allItems: Item[]; item: Item | null; onClose: () => void; onSaved: () => void }) {
  ...
  const similarItems = !item && normalizedName.length >= 2 ? allItems.filter((entry) => {
  ...
  return <><Modal title={item ? "编辑物品" : "录入新物品"} subtitle={item?.itemCode || "只填名称和数量也可以，其他信息稍后补充"} onClose={onClose}><form onSubmit={submit} className="space-y-4">
    ...
    {(duplicates.length > 0 || similarItems.length > 0 || commonTemplate) && <div className="space-y-1.5 rounded-xl px-3 py-2 text-xs" style={{ background: "var(--warning-soft, #fff4dc)", color: "#8a571c" }}>{duplicates.length > 0 && <div className="flex items-center gap-2"><CircleAlert size={15} className="shrink-0" /><span className="min-w-0 flex-1 truncate">已有同名物品：{duplicates.slice(0, 2).map((entry) => entry.location?.name || "未设置位置").join("、")}；仍可继续新增。</span></div>}{similarItems.length > 0 && <div className="flex items-center gap-2"><CircleAlert size={15} className="shrink-0 opacity-75" /><span className="min-w-0 flex-1 truncate">可能已有相似物品：{similarItems.map((entry) => entry.name).join("、")}</span></div>}{commonTemplate && <div className="flex items-center gap-2"><Sparkles size={15} className="shrink-0" /><span className="min-w-0 flex-1 truncate">识别为常见{commonTemplate.category}用品{suggestedLocation ? `，可放在 ${suggestedLocation.name}` : ""}</span><button type="button" onClick={applyTemplate} className="shrink-0 font-black underline underline-offset-2">一键采用</button></div>}</div>}
    {draftAi.hasHighlights && <div className="rounded-2xl border p-3" style={{ borderColor: "color-mix(in srgb, var(--primary) 24%, var(--border))", background: "color-mix(in srgb, var(--primary) 5%, var(--surface-solid))" }}><div className="flex items-center gap-1.5 text-xs font-black" style={{ color: "var(--primary)" }}><Sparkles size={14} />AI 信息</div>{draftAi.summary && <p className="mb-0 mt-2 text-xs leading-5">{draftAi.summary}</p>}<div className="mt-2 grid gap-2 sm:grid-cols-2">{draftAi.storage && <div className="text-xs leading-5"><span className="font-bold">存储：</span>{draftAi.storage}</div>}{(draftAi.usage || draftAi.replenishment) && <div className="text-xs leading-5"><span className="font-bold">使用与补货：</span>{[draftAi.usage, draftAi.replenishment].filter(Boolean).join("；")}</div>}</div></div>}
...
function LocationModal({ location, locations, onClose, onSaved }: { location: Location | null; locations: Location[]; onClose: () => void; onSaved: () => void }) { const [name, setName] = useState(location?.name || ""); const [color, setColor] = useState(location?.color || "#7c3aed"); const [parentId, setParentId] = useState(location?.parentId || ""); const [error, setError] = useState(""); const submit = async (e: FormEvent) => { e.preventDefault(); setError(""); try { await request(location ? `/api/locations/${location.id}` : "/api/locations", { method: location ? "PATCH" : "POST", body: JSON.stringify({ name, color, parentId: parentId || null, icon: location?.icon || "Package" }) }); onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : "保存失败"); } }; return <Modal title={location ? "编辑家庭空间" : "添加家庭空间"} subtitle="房间、柜子或任何方便查找的位置。" onClose={onClose}><form onSubmit={submit} className="space-y-4"><Field label="空间名称"><input autoFocus required className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：主卧衣柜" /></Field><Field label="所属上级空间"><select className="input" value={parentId} onChange={(e) => setParentId(e.target.value)}><option value="">顶级空间</option>{locations.filter((entry) => entry.id !== location?.id).map((entry) => <option key={entry.id} value={entry.id}>{entry.parent?.name ? `${entry.parent.name} · ` : ""}{entry.name}</option>)}</select></Field><Field label="标记颜色"><div className="flex items-center gap-3"><input type="color" className="h-11 w-16 cursor-pointer rounded-xl border-0 bg-transparent" value={color} onChange={(e) => setColor(e.target.value)} /><span className="text-sm muted">用于快速区分空间</span></div></Field>{error && <p className="m-0 rounded-xl p-2.5 text-sm text-red-500" style={{ background: "#ffe8eb" }}>{error}</p>}<div className="flex gap-3 pt-2"><button type="button" onClick={onClose} className="btn-ghost flex-1">取消</button><button className="btn-primary flex-1">{location ? "保存修改" : "创建空间"}</button></div></form></Modal>; }
