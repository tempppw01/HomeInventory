  ...
  return (
    <div className={`min-h-screen md:grid ${sidebarCollapsed ? "md:grid-cols-[76px_minmax(0,1fr)]" : "md:grid-cols-[232px_minmax(0,1fr)]"}`}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .22 }}>
            {loading ? <LoadingView /> : view === "dashboard" ? (
              ...
              <ItemsView allItems={data!.items} locations={data!.locations} items={filteredItems} shopping={data!.shopping} search={search} setSearch={setSearch} filter={typeFilter} setFilter={setTypeFilter} onEdit={openEdit} onConsume={consume} onRemainingChange={updateRemaining} onDelete={removeItem} onQr={setQrItem} onAi={setAiItem} onPrint={setPrintItems} onToast={setToast} onCopy={(item) => copyText(item.itemCode || item.id, "物品编号已复制")} onToggleShopping={toggleShopping} onAddShopping={() => setModal("shopping")} onDeleteShopping={async (id) => { await request(`/api/shopping/${id}`, { method: "DELETE" }); await refresh(); }} />
            ) : view === "locations" ? (
      ...
      <AnimatePresence>
        ...
        {modal === "notifications" && <NotificationsModal lowStock={lowStock} expiring={expiring} expired={expired} onClose={closeModal} onOpenItem={(item) => { closeModal(); openEdit(item); }} onShopping={() => { closeModal(); setView("items"); }} />}
...
function ItemsView({ allItems, locations, items, shopping, search, setSearch, filter, setFilter, onEdit, onConsume, onRemainingChange, onDelete, onQr, onAi, onPrint, onToast, onCopy, onToggleShopping, onAddShopping, onDeleteShopping }: { allItems: Item[]; locations: Location[]; items: Item[]; shopping: ShoppingItem[]; search: string; setSearch: (s: string) => void; filter: "ALL" | ItemType; setFilter: (f: "ALL" | ItemType) => void; onEdit: (i: Item) => void; onConsume: (i: Item) => void; onRemainingChange: (item: Item, remainingPercent: number) => void; onDelete: (i: Item) => void; onQr: (i: Item) => void; onAi: (i: Item) => void; onPrint: (items: Item[]) => void; onToast: (message: string) => void; onCopy: (i: Item) => void; onToggleShopping: (item: ShoppingItem) => void; onAddShopping: () => void; onDeleteShopping: (id: string) => void }) {
  ...
  return <><PageTitle title="物品与采购" text="知道家里有什么，也记得还要买什么。" action={<button onClick={onAddShopping} className="btn-ghost hidden items-center gap-1.5 text-xs sm:flex"><ShoppingBasket size={15} />添加采购项</button>} />
    ...
    <section className="mt-7 border-t pt-5" style={{ borderColor: "var(--border)" }}><div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="m-0 text-base font-black">采购清单</h2><p className="mb-0 mt-1 text-xs muted">{pendingShopping.length ? `${pendingShopping.length} 项等你顺路带回家` : "暂时没有需要采购的东西"}</p></div><button onClick={onAddShopping} className="icon-action" aria-label="添加采购项" title="添加采购项"><Plus size={18} /></button></div>{pendingShopping.length ? <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{pendingShopping.map((item) => <ShoppingRow key={item.id} item={item} onToggle={() => onToggleShopping(item)} onDelete={() => onDeleteShopping(item.id)} />)}</div> : <button onClick={onAddShopping} className="flex items-center gap-2 text-xs font-bold" style={{ color: "var(--primary)" }}><Plus size={15} />添加第一项采购</button>}</section>
